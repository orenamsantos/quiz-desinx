...
function goCheckout() {
  ...
  const checkoutLinks = {
    essential: 'https://checkout.ticto.app/SEU_LINK_ESSENTIAL',
    premium:   'https://checkout.ticto.app/SEU_LINK_PREMIUM',
  };

  let url = checkoutLinks[plan] || checkoutLinks.premium;

  // Pre-fill name and phone from quiz data
  const params = new URLSearchParams();
  if (state.userData.name) params.set('name', state.userData.name);
  if (state.userData.whatsapp) params.set('phone', state.userData.whatsapp);
